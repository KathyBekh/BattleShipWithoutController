...
class ComputerPlayer(private val board: Field) {
    ...
    private fun evaluation(side: Ship = Ship.BLUE): Int {
        if (side == Ship.RED) return -evaluation(Ship.BLUE)
            ...
            for (x in 0 until width) {
                for (y in 0 until height) {
                    ...
                    for (dir in directions) {
                        val finish = start + dir * (Field.TO_WIN_LENGTH - 1)
